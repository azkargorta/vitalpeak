(() => {
  ...
  function injectStyles(){
    ...
    const s=document.createElement('style');s.id=STYLE_ID;s.textContent=`
      ...
      .vp-share-range{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:8px;margin-bottom:14px}.vp-share-range label{min-width:0;font-size:11px;font-weight:800;color:#62797f}.vp-share-range input{display:block;box-sizing:border-box;min-width:0;width:100%;max-width:100%;margin-top:5px;border:1px solid #cfe2de;border-radius:12px;padding:9px 7px;background:#fff;color:#173b43;font-size:12px;font-weight:800;text-align:center}
      ...
      @media(max-width:620px){.vp-share-backdrop{padding:0;align-items:flex-end}.vp-share-dialog{border-radius:24px 24px 0 0;max-height:90vh;padding:18px 16px}.vp-share-range{gap:7px}.vp-share-range input{font-size:11px;padding:9px 4px}.vp-share-summary div{padding:11px 8px}.vp-share-summary b{font-size:19px}.vp-share-summary span{font-size:9px}}
  ...
  function renderPreview(state,from,to){
    ...
    root.innerHTML=`<div class="vp-share-summary"><div><b>${data.sessions.length}</b><span>entrenamientos</span></div><div><b>${data.setRows.length}</b><span>series</span></div><div><b>${data.progress.length}</b><span>ejercicios</span></div></div><p class="vp-share-note">El detalle completo de ejercicios, series, cargas, repeticiones y peso corporal se incluirá en el archivo compartido.</p>`;
    return data;
  }

  function csvEscape(v){const s=String(v??'');return /[;"\n]/.test(s)?`"${s.replace(/"/g,'""')}"`:s}
  function makeCsv(data,from,to){
    const header=['Tipo','Desde','Hasta','Fecha','Rutina','Ejercicio','Serie','Peso ejercicio kg','Repeticiones','Sesiones ejercicio','Series ejercicio','Mejor inicial kg','Mejor final kg','Cambio ejercicio kg','Peso corporal kg','Cambio peso corporal kg','Notas'];
    const rows=[header];
    data.setRows.forEach(r=>rows.push(['ENTRENAMIENTO',from,to,r.date,r.routine,r.exercise,r.set,r.weight,r.reps,'','','','','','','',r.notes]));
    data.progress.forEach(r=>rows.push(['PROGRESO EJERCICIO',from,to,'','',r.exercise,'','','',r.sessions,r.sets,r.firstBest,r.lastBest,r.change,'','','']));
    const firstWeight=data.weights[0]?.kg;
    data.weights.forEach((w,i)=>rows.push(['PESO CORPORAL',from,to,w.date,'','','','','','','','','','',w.kg,i===0?0:num(w.kg)-num(firstWeight),'']));
    if(rows.length===1)rows.push(['SIN DATOS',from,to,'','','','','','','','','','','','','','']);
