(() => {
  ...
  async function enhanceRecords(){
    ...
    const list=page?.querySelector('[data-vp-record-list]');
    if(!page||!list)return;
    const state=await readState().catch(()=>({sessions:[]}));
    const all=records(state);
    const expanded=sessionStorage.getItem(RECORDS_KEY)==='1';
    ...
    if(all.length>3){
      if(!toggle){
        ...
        toggle.type='button';toggle.className='vp-link vp-record-toggle-fixed';toggle.dataset.vpToggleRecords='';
        list.insertAdjacentElement('afterend',toggle);
      }
      toggle.innerHTML=`${expanded?'Ver menos':'Ver más'} ${chev}`;
    }else toggle?.remove();
  }

  async function enhanceWeights(){
    const page=document.querySelector('.vp-progress-page');
    const form=page?.querySelector('.vp-weight-form');
    if(!page||!form)return;
    let box=page.querySelector('.vp-weight-history');
    if(!box){box=document.createElement('div');box.className='vp-weight-history';form.insertAdjacentElement('afterend',box)}
  ...
  async function enhance(){
    if(enhancing)return;
    if(!document.querySelector('.tabbar button[data-route="progress"].active'))return;
    if(!document.querySelector('.vp-progress-page'))return;
    enhancing=true;try{injectStyles();await Promise.all([enhanceRecords(),enhanceWeights()])}finally{enhancing=false}
  }

  function rerenderProgress(){
    const app=document.querySelector('#app');
    app?.classList.remove('vp-progress-ready');
  ...
  async function toggleRecords(e){
    const button=e.target.closest('.vp-progress-page [data-vp-toggle-records]');
    if(!button)return false;
    e.preventDefault();e.stopImmediatePropagation();
    const expanded=sessionStorage.getItem(RECORDS_KEY)==='1';
    sessionStorage.setItem(RECORDS_KEY,expanded?'0':'1');
  ...
  document.addEventListener('click',async e=>{
    ...
    if(edit){
      e.preventDefault();e.stopImmediatePropagation();
      const index=Number(edit.dataset.vpEditWeight),row=edit.closest('[data-vp-weight-row]'),state=await readState();const current=state.weights?.[index];if(!row||!current)return;
      row.outerHTML=`<div class="vp-weight-edit-row" data-vp-weight-edit-row="${index}"><input type="number" inputmode="decimal" step="0.1" min="1" value="${num(current.kg)}" aria-label="Peso en kg"><button type="button" class="vp-weight-save" data-vp-save-weight="${index}">Guardar</button><button type="button" class="vp-weight-cancel" data-vp-cancel-weight>Cancelar</button></div>`;
      document.querySelector(`[data-vp-weight-edit-row="${index}"] input`)?.focus();return;
    }
    ...
    const save=e.target.closest('[data-vp-save-weight]');if(save){
      e.preventDefault();e.stopImmediatePropagation();const index=Number(save.dataset.vpSaveWeight),row=save.closest('[data-vp-weight-edit-row]'),kg=Number(row?.querySelector('input')?.value);if(!Number.isFinite(kg)||kg<=0){row?.querySelector('input')?.focus();return}
      const state=await readState();if(!Array.isArray(state.weights)||!state.weights[index])return;state.weights[index]={...state.weights[index],kg};await writeState(state);rerenderProgress();return;
    }
    const del=e.target.closest('[data-vp-delete-weight]');if(del){
      e.preventDefault();e.stopImmediatePropagation();const index=Number(del.dataset.vpDeleteWeight),state=await readState();if(!Array.isArray(state.weights)||!state.weights[index])return;const w=state.weights[index];if(!confirm(`¿Eliminar el registro de ${fmt(w.kg)} kg?`))return;state.weights.splice(index,1);await writeState(state);rerenderProgress();return;
    }
  },true);

  document.addEventListener('submit',e=>{if(e.target.closest('.vp-progress-page .vp-weight-form'))setTimeout(()=>{rerenderProgress()},160)},false);
