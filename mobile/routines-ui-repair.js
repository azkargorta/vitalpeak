(() => {
  ...
  function fixAddExerciseIcon(){
    ...
    if(!icon||icon.dataset.vpCorrectIcon==='1')return;
    icon.dataset.vpCorrectIcon='1';
  ...
  function addFilters(body,grid){
    ...
    const f=document.createElement('div');f.className='vp-section-filters';f.dataset.vpRepairFilters='1';
  ...
  function repairExerciseCatalog(){
    ...
    if(!grid.querySelector('.exercise-card')){
      ...
      body.dataset.vpCatalogRepaired='1';
    }
  ...
  function repair(){
    ...
    fixAddExerciseIcon();repairExerciseCatalog();
  }
  function schedule(){clearTimeout(timer);timer=setTimeout(repair,90)}
