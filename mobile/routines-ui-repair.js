(() => {
  ...
  const ADD_ICON='<span aria-hidden="true" style="font-size:28px;line-height:1;font-weight:500;display:block;transform:translateY(-1px)">+</span>';
  ...
  function fixAddExerciseIcon(){
    ...
    if(!icon)return;
    if(icon.dataset.vpCorrectIcon==='3')return;
    icon.innerHTML=ADD_ICON;
    icon.dataset.vpCorrectIcon='3';
  }
  ...
  function addFilters(body,grid){
    ...
    const f=document.createElement('div');f.className='vp-section-filters';f.dataset.vpRepairFilters='3';
  ...
  function bindExerciseSection(){
    ...
    if(summary.dataset.vpCatalogToggle==='3')return;
    summary.dataset.vpCatalogToggle='3';
  ...
  function schedule(delay=35){clearTimeout(timer);timer=setTimeout(repair,delay)}

  const app=document.getElementById('app');
  if(app)new MutationObserver(()=>schedule()).observe(app,{childList:true,subtree:false});
  const tabs=document.querySelector('.tabbar');
  if(tabs)new MutationObserver(()=>schedule(20)).observe(tabs,{attributes:true,subtree:true,attributeFilter:['class']});
  document.addEventListener('click',e=>{if(e.target.closest('[data-route="routines"]'))schedule(20)},true);
